...
public:
    string simplifyPath(string path) {
        ...
        stack<string> auxPathStack;
        istringstream ss(path);
        while (getline(ss, tmpDirectory, '/'))
        {
            cout << tmpDirectory << endl;
            if (tmpDirectory == "..")
            {
            ...
            {
                 if (tmpDirectory != "." && tmpDirectory!="")    PathStack.push(tmpDirectory);
            }
        }
        while (!PathStack.empty())
        {
            auxPathStack.push(PathStack.top());
            PathStack.pop();
        }
        if (auxPathStack.empty())
            return "/";
        while(!auxPathStack.empty())
        {
            returnstring += ("/"+auxPathStack.top());
            auxPathStack.pop();
        }
        return returnstring;

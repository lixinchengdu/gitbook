...
* *Similar Questions:*
  ...
  * [Populating Next Right Pointers in Each Node](./tests/populating-next-right-pointers-in-each-node-ii.md)
...
public:
    void connect(TreeLinkNode *root) {
        while (root)
        {
            ...
            TreeLinkNode *nextLevel = dummyHead;
            while(root)
            {
                if (root->left)
                {
                    nextLevel -> next = root->left;
